...
- **顶部透视表 V4 优化**（2026-08-17）：0 值不显示 + 可点击跳转
  - 分类胶囊格式 `图标 + 短名（2字）+ 数字`（如 `👑 元首 17`），空分类隐藏
  - 点击分类胶囊 → 跳转到"该日期+该分类"新闻；点击行/总计 → 跳转"该日期全部"
  - 跳转行为：同步顶部日期按钮 + 左侧栏目栏 + 清空搜索/筛选 + 滚动到表格
  - 同样逻辑同步到 AI 看板 build_v2.py（产业/模型/技巧）
