...
def build():
    ...
    # 顶部统计表（AI 用 日期×分类 透视 · V5 透视表 · 日期在左、分类在上、中间仅数字）
    ...
    for c in cat_keys:
        thead_cells.append(
            f'<th class="pivot-th" title="{html.escape(c)}">{html.escape(cat_short.get(c, c))}</th>'
        )
    ...
    for d in dates:
        ...
        for c in cat_keys:
            ...
            if cnt:
                cells_html.append(
                    ...
                    f'title="点击查看 {d} · {c}（{cnt} 条）">{cnt}</span>'
                    f'</td>'
                )
            else:
        ...
        pivot_rows.append(
            f'<tr class="{row_cls}" data-date="{html.escape(d)}" '
            f'title="点击查看 {d} 全部新闻（{total_row} 条）">'
            f'<td class="pivot-td date"><b>{date_label}</b><span class="weekday">{wd}</span>{today_dot}</td>'
            f'{"".join(cells_html)}'
            f'<td class="pivot-td total"><span class="pivot-total" data-date="{html.escape(d)}" '
