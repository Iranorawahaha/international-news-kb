...
# 部委关键词（V4.1 用户反馈：AI/芯片等纯技术话题不归部委，部委须为具体行动/声明）
BUWEI_KEYWORDS = [
    # 用户点名部委（直接出现在标题中）
    "工信部", "工业和信息化部", "网信办", "国家网信办", "中央网信办", "发改委",
    "国家发展改革委", "发展改革委", "国办", "国务院办公厅", "中办", "中共中央办公厅",
    "商务部", "外交部", "外交部发言人",
    # 部委特定行动（政策/调查/执法/声明）—— 必须是部委行为，不是泛行业话题
    "对外贸易", "国家安全调查", "反倾销", "反补贴", "保障措施调查",
    "中美贸易", "贸易战", "经济制裁", "实体清单", "出口管制", "关税",
    # AI/信息通信领域——仅保留部委具体监管动作，删除泛泛的 AI/芯片/5G 等（它们应归经贸）
    ...
    for kw in XI_KEYWORDS:
        ...
        for kw in PSC_KEYWORDS:
            ...
            if cat == "使领馆动向":
                ...
                # V4.1: 重要部委行动加权——对外贸易调查/反倾销/反补贴等商务部重大执法 → 高分
                if any(k in title for k in ["对外贸易", "国家安全调查", "反倾销", "反补贴", "保障措施调查", "实体清单", "出口管制"]):
                    score = 88
                else:
                    score = 78
            elif cat == "政策发布":
