...
V5.8（2026-09-02 用户拍板"四类问题诊断正确，请动手"，基于 216 条存量回归验证）：
  - ✅ 央视随行特稿/时政解读栏目硬排除：大国外交最前线/时政新闻眼/平语近人/独家视频/学习卡/传习录/习语
    （原 normalize_title_for_dedup 只剥"丨"前缀不去重剔除，9-02 上合行程堆叠 19 条元首动态的根因）
  - ✅ 外方主体涉华报道分流（is_foreign_subject 双轨：表态/政策行为；豁免中方主场/回应）——
    固化 9-02 手动先例（德国对华方案/美财长评G20），此类由国际版外媒覆盖
  - ✅ 配偶活动归元首动态（彭丽媛 classify 特判 + 92 分线；原被"峰会"吸进重要会议）
  - ✅ 经贸动向拆杂烩：产业技术/能源里程碑归部委（产业化突破/光伏发电…）；
    招聘专场/综述体剔除；国台办归部委
  - ✅ 词表冲突修复（回归发现）：JUNK 天气灾害词整体移除（防杀元首救灾指示/重大灾害，
    灾害由 BUWEI 国家级救灾词收录）；MEETING 删"改革委"裸词（防抢发改委）；
    PSC 删"全国人大/全国政协"裸词（人大政协事务归会议线）；政策删"规定"裸词（防八项规定误伤）；
    MEETING 补"常委会"（政协十四届常委会全称式）；PERSONNEL 补已任/免职/八项规定/党组书记
  - ⚠️ 只影响新抓（9-03 起），历史 archive 不回改
"""
import json
import os
import re
...
PSC_KEYWORDS = ["李强", "赵乐际", "王沪宁", "蔡奇", "丁薛祥", "李希",
                ...
                # V5 扩展：更多高层职务/活动信号
                # V5.8: 删除裸"全国人大/全国政协"——会抢走人大政协事务
                #   （人大常委会立法/任免应归 重要会议/人事任免，见 MEETING 的"全国人大常委/全国政协常委会"）
                "国务院总理", "中央军委", "国务委员",
...
BUWEI_KEYWORDS = [
    ...
    # 5. 部委通用行动信号
    "约谈", "通报", "答记者问", "新闻发布会",
    # V5.8: 国台办（涉台官方机构，案例 9-02"台胞金融服务中心"原落经贸）
    "国台办", "国务院台办",
    # V5.8: 国家级灾害救援（用户规则：灾情应急→部委88；救灾资金→部委85）
    #   案例 8-27 央视吉隆"国家二级救灾应急响应启动"原 classify None 丢失
    #   ⚠️ JUNK 天气灾害词已删（防杀元首救灾指示）；此处补收录线——
    #   地方小灾（无"救灾/应急响应/应急管理部"国家级信号词）仍自然过滤
    "应急管理部", "国家防总", "防汛抗旱", "救灾", "抢险救援", "应急响应",
    # 6. 重大产业/科技政策（归部委）
    "算力网", "算力节点", "全国算力", "一体化算力", "东数西算",
    "算力调度", "智算中心", "超算中心", "算力枢纽", "算力集群",
    # 7. 网信/工信监管动作
    "专项整治", "集中整治", "联合检查",
    # V5.8: 产业技术/能源结构里程碑 → 部委线（工信部/能源局领域，防经贸动向成杂烩）
    #   案例 9-02：31 难选铁矿产业化突破、36 光伏装机首超煤电
    "产业化突破", "光伏发电", "新型电力系统", "新型储能", "关键核心技术突破",
]
...
PERSONNEL_KEYWORDS = [
    ...
    "拟任", "提拔", "晋升", "升任", "兼任", "挂职",
    # V5.8: 补词（回归发现漏判）
    #   "已任" → 案例 9-02 华列兵"已任…纪检组组长"（原 classify None 丢弃）
    #   "免职" → 案例 8-29"人大常委会决定免职的名单"（原被政策"决定"抢走）
    #   "八项规定" → 案例 8-28 中央纪委每月作风通报"查处违反中央八项规定精神问题"
    #     （原被政策"规定"裸词抢走；PERSONNEL 在政策前命中 → 归反贪腐线 87 分）
    #   "党组书记" → 案例 8-29"刘桂平任中国市场监管总局党组书记"（原落经贸）
    "已任", "免职", "八项规定", "党组书记",
    # 机构任命
...
MEETING_KEYWORDS = [
    ...
    "全会", "常务会议", "国务院会议", "政治局会议",
    # V5 扩展
    "国务院常务会议", "中央政治局", "政治局常委", "深改委",
    # V5.8: 删除裸"改革委"——会撞"国家发展改革委"→错误归重要会议（回归 8-27 发改委救灾资金）
    "中央财经委", "中央军委", "中央外事", "中央农村工作", "中央统战",
    "全国政协常委会", "全国人大常委", "委员长会议", "主席会议",
    # V5.8: 加"常委会"裸词——覆盖"全国政协十四届常委会第十八次会议"格式
    #   （"全国政协常委会"词序含届次时不命中，PSC 删除"全国政协"后 classify None 丢失 8-27 案例）
    "常委会",
...
JUNK_KEYWORDS = [
    ...
    # V5.8: 天气/灾害词已整体从 JUNK 移除（含台风/暴雨/洪水/灾情/应急响应等）
    #   根因：is_junk 在 classify 之前执行，而元首/国务院救灾指示类标题必然含灾害词
    #   （如"习近平对防汛抗洪救灾作出重要指示""启动国家Ⅳ级救灾应急响应"）
    #   → 这些词会连重大灾害/元首指示一起误杀（回归验证 8-27 吉隆、8-28 台风Ⅰ级均中招）
    #   替代防线：日常天气预报文 classify 无任何分类词命中 → 自然返回 None 丢弃，
    #   重大灾害（含部委/财政/元首词）正常收录，符合用户规则（灾情应急→部委88）。
    # 文旅/非遗/民俗/民生杂项
    ...
    #   地方微观服务（"个人身后金融事"）
    "身后金融事", "个人身后", "一站式查询系统",
    # V5.8: 央视随行特稿/时政解读栏目（非实事新闻，内容与主新闻重复堆叠）
    #   案例 9-02：15/16 大国外交最前线、17 时政新闻眼、14 平语近人(阿语版启播)
    #   ⚠️ normalize_title_for_dedup 只剥"丨"前缀去重不剔除 → 必须在此硬排除
    "大国外交最前线", "时政新闻眼", "平“语”近人", '平"语"近人', "平语近人",
    "独家视频", "学习卡丨", "传习录丨", "习语丨",
    # V5.8: 经济形势评论/综述体（用户规则：评述/数据综述/回顾类排除）
    #   案例 9-02：30《"稳"：看中国经济为世界经济注入宝贵确定性》
    "综述", "注入宝贵确定性", "经济述评",
    # V5.8: 民生服务稿（就业招聘专场等常规服务，非重点部委动态）
    #   案例 9-02：28 @求职者 线上招聘专场
    "招聘专场", "求职者",
]
...
# V5.8: 外方主体涉华报道 → 不收录国内版（此类事件由国际版外媒权威源覆盖）
# 固化 9-02 手动先例：27 德国拟对华贸易方案、32 美财长评 G20 → 移出国内版
# 轨A（表态/引语）：FOREIGN_SAYER 命中 + 涉华对象（中国/对华/涉华）
# 轨B（政策行为）：FOREIGN_ACTOR 命中 + 明确客体（对华/涉华/针对中国…）
# ⚠️ 豁免 FOREIGN_KEEP 先判：中方在场/主场（访华/会见/中方回应）绝不剔除
FOREIGN_SAYER = [
    "美财长", "美国财政部长", "美国商务部长", "美国国务卿", "美国贸易代表",
    "美联储", "白宫", "五角大楼", "美国国务院", "美媒", "英媒", "德媒", "日媒",
    "欧盟委员会", "欧委会", "北约秘书长", "G7", "七国集团", "美中贸易全国委员会",
    "美国财政部长耶伦", "美国国务卿鲁比奥",
]
FOREIGN_ACTOR = [
    "德国", "法国", "英国", "日本", "韩国", "澳大利亚", "加拿大", "荷兰",
    "欧盟", "美国", "印度",
]
FOREIGN_KEEP = [
    # 中方在场/主场/回应 → 保留
    "习近平", "中方", "中国外交部", "外交部发言人", "我外交部", "我商务部", "国台办",
    "外交部", "商务部", "国务院关税税则委员会", "中国驻", "我驻",
    # 主场外交动作
    "访华", "到访", "应邀", "会见", "会谈", "通话", "应约",
    "同中国", "与中国", "在北京",
    # 中方胜诉/裁决结果（非外方单方表态）
    "胜诉", "裁决", "裁定",
]
...
def is_foreign_subject(title):
    """V5.8: 外方主体涉华报道过滤（见 FOREIGN_SAYER/ACTOR 注释）"""
    t = title or ""
    # 豁免优先：中方主体在场或主场外交 → 保留
    if any(k in t for k in FOREIGN_KEEP):
        return False
    # 轨B：外方对华政策动作（需明确"对华"客体，防误杀"德国总理访华"类主场）
    if any(k in t for k in FOREIGN_ACTOR) and any(
        k in t for k in ("对华", "涉华", "针对中国", "对中国的", "就中国",
                         "批评中国", "指责中国", "制裁中国", "限制中国", "打压中国")
    ):
        return True
    # 轨A：外方表态/引语涉华（含"中国"裸词即可，豁免词已排除中方主场）
    if any(k in t for k in FOREIGN_SAYER) and any(
        k in t for k in ("中国", "对华", "涉华", "北京")
    ):
        return True
    return False
...
CATEGORY_RULES = [
    ...
    ("部委动态", BUWEI_KEYWORDS),
    # 6. 政策发布
    # V5.8: 删除裸"规定"——"八项规定/党章规定/按规定"大量误伤（8-28 纪检通报案例）；
    #       "XX规定"文件标题必带"印发/条例/办法"等前词，不受影响
    ("政策发布", ["印发", "通知", "规划", "意见", "方案", "条例", "办法", "决定", "批复", "白皮书"]),
    # 7. 经贸动向
...
def classify(title):
    # V5.8: 配偶随行活动 → 元首动态（标题可无"习近平"）
    #   案例 9-02：22 彭丽媛出席上合峰会配偶集体活动（原被"峰会"吸进重要会议）
    if "彭丽媛" in title and any(k in title for k in ("出席", "参加", "会见", "共同", "陪同", "参观")):
        return "元首动态"
        ...
        for kw in kws:
            if kw in title:
                if cat == "重要会议":
                    if "议会" in title:
                        continue  # 跳过：非国内重要会议
                    # V5.8: "常委会"裸词命中但为地方层级（省/市人大常委会）→ 跳过
                    #   防联早地方人大任免被抢（案例 8-29 覃伟中履新广东省人大常委会领导 → 人事）
                    if kw == "常委会" and any(p in title for p in ("省人大", "市人大")):
                        continue
                return cat
...
def assess_importance(title, cat):
    """重要度 0-100"""
    # V5.8: 配偶随行活动 → 元首线次级（92，略低于元首本人 100）
    if "彭丽媛" in title and cat == "元首动态":
        return 92
    score = 50
...
def make_item(title, url, date, source, summary=None):
    ...
    if is_comment_column(title):
        return None
    # V5.8: 外方主体涉华报道 → 不收录（国际版外媒权威源覆盖，固化 9-02 手动分流先例）
    if is_foreign_subject(title):
        return None
