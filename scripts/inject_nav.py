...
# -*- coding: utf-8 -*-
"""
inject_nav.py — Ira 信息看板体系 统一导航注入脚本（v2）
幂等：若目标 HTML 已包含 NAV-MARKER，则跳过，不重复注入。
处理对象：international-news.html（国际新闻看板）、ai-company-intel.html（AI 动向看板）
导航结构：Ira 信息看板（门户） | 国际新闻看板 | AI 动向看板
用法: python3 scripts/inject_nav.py [html路径...]
     不传路径时默认处理项目根目录与 gh-pages/ 下的两个子看板页面
"""
import sys, os, re
...
<div class="ira-nav" style="max-width:1400px;margin:14px auto 0;padding:9px 18px;background:#fff;border:1px solid #e4e7ee;border-radius:12px;box-shadow:0 2px 8px rgba(28,36,52,.06);display:flex;align-items:center;gap:6px;flex-wrap:wrap;font-size:13px;">
  <span style="font-weight:700;color:#1c2434;margin-right:4px;background:linear-gradient(120deg,#3b4a8c,#b4251a);-webkit-background-clip:text;background-clip:text;color:transparent;">Ira 信息看板</span>
  ...
  <span style="color:#a0a8b5;margin-left:auto;font-size:12px;">每日 09:40 自动刷新</span>
</div>
<!-- ===== /IRA-NAV ===== -->'''
...
def main():
    targets = sys.argv[1:] if len(sys.argv) > 1 else [
        ...
        os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "ai-company-intel.html"),
        os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "gh-pages", "ai-company-intel.html"),
    ]
    for t in targets:
